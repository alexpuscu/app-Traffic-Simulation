...
class NavigationManager(object):
    ...
    def __init__(self, car, max_speed, acceleration, route, error, distance_acceptance):
        self.car = car
    ...
    def update_position(self, delta_t):
        if self.state == State.CRUISING:
            ...
            self.y += self.speed_y * delta_t + 0.5 * self.acc_y * delta_t ** 2
            self.speed_y += self.acc_y * delta_t
            self.x += self.speed_x * delta_t + 0.5 * self.acc_x * delta_t ** 2
    ...
    def analyze_update(self):
        ...
        if self.on_end_of_block(block) and expected_block == block and self.route.index != len(self.route.blocks) - 1:
    ...
    def analyze_travel_end(self):
        if self.route.index == len(self.route.blocks) - 1 and not self.arrived and self.has_arrived():
            self.arrived = True
            self.set_original_acc()
            self.acc_x, self.acc_y = -self.acc_x, -self.acc_y
            self.state = State.BREAKING
        if self.arrived and self.state == State.STOPPED:
    ...
    def out_of_city(self):
        return self.x < -2 * self.error or self.x > self.car.city.width + 2 * self.error or self.y < -2 * self.error or self.y > self.car.city.height + 2 * self.error
    ...
    def no_need_breaking(self, other):
        if other.navigation_manager.current_road() != self.current_road():
            if is_horizontal(self.current_road().direction):
                return abs(self.speed_x) < abs(other.navigation_manager.speed_y)
            else:
                return abs(self.speed_y) < abs(other.navigation_manager.speed_x)
        else:
            if is_horizontal(self.current_road().direction):
                return abs(self.speed_x) < abs(other.navigation_manager.speed_x)
            else:
                return abs(self.speed_y) < abs(other.navigation_manager.speed_y)
    ...
    def get_distance(self, other):
        if other.navigation_manager.current_road() != self.current_road():
            d = abs(self.x - other.navigation_manager.x) + abs(self.y - other.navigation_manager.y)
        elif self.current_road().direction == Direction.WE:
...
class CommunicationManager(object):
    ...
    def process_answers(self):
        ...
        while self.car.answers:
            ...
            if ans.m_type == MessageType.DISTANCE and ans.msg[0] < self.distance_acceptance and (not most_important_distance_msg or ans.msg[0] < most_important_distance_msg.msg[0]):
            ...
            if ans.m_type == MessageType.INTERSECTION:
                ...
                if d > 3 * constants.CAR_RADIUS:
                    ...
                    if not most_important_distance_msg or d < most_important_distance_msg.msg[0]:
                        most_important_distance_msg = Response(MessageType.DISTANCE, [
                         d - 3 * constants.CAR_RADIUS, None])

        if most_important_distance_msg:
            self.car.navigation_manager.process_break(most_important_distance_msg.msg[0], most_important_distance_msg.msg[1])
        else:
    ...
    def process_requests(self):
        while self.car.requests:
            ...
            if req.m_type == MessageType.DISTANCE and self.car.navigation_manager.before(req.requester):
                d = self.car.navigation_manager.effective_distance(req.requester)
                if d <= 0:
                    self.car.city.inform_crash(self.car, req.requester)
                else:
                    req.requester.answers.append(Response(MessageType.DISTANCE, [
                     d * 0.5, self.car]))
                if req.m_type == MessageType.INTERSECTION and self.car.navigation_manager.passed_half_block():
    ...
    def make_requests(self):
        ...
        if priority_block and self.car.navigation_manager.passed_half_block():
            for each in priority_block.cars:
                if not each == self.car:
                    each.requests.append(Request(self.car, MessageType.INTERSECTION))


class Car(Agent):
    counter = 0

    def __init__(self, city, route, max_speed, acceleration, error, distance_acceptance):
        super(Car, self).__init__()
        self.id = Car.counter
        Car.counter += 1
        self.city = city
        self.navigation_manager = NavigationManager(self, max_speed, acceleration, route, error, distance_acceptance)
        self.communication_manager = CommunicationManager(self, distance_acceptance)
